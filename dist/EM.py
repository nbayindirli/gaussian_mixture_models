...
class EM:
    ...
    def run(self, max_steps=100, test_data=None):
        ...
        while num_iters < max_steps and not converged:
            ...
            test_likelihood.append(new_log_likelihood)
...
if __name__ == "__main__":
    ...
    for i in range(int(clusters)):
        ...
        if len(sys.argv) >= 1:
            ...
            e = EM(d, 3)
        (train_likelihood, test, iterations) = e.run(100)
        all_train_LL.append(train_likelihood[len(train_likelihood) - 1])
    ...
    for i in range(int(clusters)):
        if len(sys.argv) >= 1:
            ...
            e = EM(d, 3)
        (train, test_likelihood, iterations) = e.run(100)
        all_test_LL.append(test_likelihood[len(test_likelihood) - 1])

    labels = read_true(filename='wine-true.data')

    plt.plot(num_clusters, all_train_LL)
    plt.plot(num_clusters, all_test_LL)
    plt.ylabel("Training/Test Log Likelihood")
