...
class EM:
    ...
    def m_step(self, log_post):
        ...
        for k in range(num_clusters):
            for d in range(num_dimensions):
                ...
                var_sum = 0
                for n in range(num_observations):
                    mean_sum += math.exp(log_post[n, k] - log_Nk[k]) * self.data[n][d]
                    var_sum += math.exp(log_post[n, k] - log_Nk[k]) * (self.data[n][d] - self.parameters[k][d][0]) * (self.data[n][d] - self.parameters[k][d][0])
                temp[k][d] = ((mean_sum, var_sum))

        self.parameters = tuple(temp)
    ...
    def log_prob(self, row, cluster, data):
        ...
        log_prob_num = self.z * data[row] * self.priors[cluster]

        log_prob_denom = 0
        for m in range(K):
            log_prob_denom += self.z[m] * data[row] * self.priors[m]

        return log_prob_num / log_prob_denom
